...
async function snapshotTable() {
  ...
  await db.execute(sql`CREATE TABLE IF NOT EXISTS ${sql.identifier(tableName)} AS TABLE persons;`);
...
async function deleteOldSnapshots() {
  ...
  for (const row of res.rows) {
    ...
    await db.execute(sql`DROP TABLE IF EXISTS ${sql.identifier(row.table_name as string)}`);
